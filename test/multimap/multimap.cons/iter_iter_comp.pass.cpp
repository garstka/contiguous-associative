...
TEST_CASE("multimap cons iter iter comp pass")
{
    {
    ...
    contiguous::multimap<int, double, C> m(ar, ar+sizeof(ar)/sizeof(ar[0]), C(5));
    ...
    typedef test_compare<std::less<int> > C;
    contiguous::multimap<int, double, C, min_allocator<V>> m(ar, ar+sizeof(ar)/sizeof(ar[0]), C(5));
    REQUIRE(m.key_comp() == C(5));
    REQUIRE(m.size() == 9);
    REQUIRE(distance(m.begin(), m.end()) == 9);
    REQUIRE(*m.begin() == V(1, 1));
