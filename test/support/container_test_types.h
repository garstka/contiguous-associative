...
#ifdef IGNORE_REQUIREMENT_23_2_1_3
	  return true;
#else
    if (!m_expected_args)
...
#ifndef IGNORE_REQUIREMENT_23_2_1_3
    assert(!unchecked());
#endif
    m_expected_args = &tid;
  }
...
template <class Key = CopyInsertable<1>, class Value = CopyInsertable<2>,
          class ValueTp = std::pair<const Key, Value> >
using map =
      contiguous::map<Key, Value, std::less<Key>,
                              ContainerTestAllocator<ValueTp, ValueTp> >;
...
template <class Key = CopyInsertable<1>, class Value = CopyInsertable<2>,
          class ValueTp = std::pair<const Key, Value> >
using multimap =
      contiguous::multimap<Key, Value, std::less<Key>,
                              ContainerTestAllocator<ValueTp, ValueTp> >;
#ifndef IGNORE_LIBCXX_SPECIFIC
...
template <class Value = CopyInsertable<1> >
using set =
    contiguous::set<Value, std::less<Value>, ContainerTestAllocator<Value, Value> >;
...
template <class Value = CopyInsertable<1> >
using multiset =
    contiguous::multiset<Value, std::less<Value>, ContainerTestAllocator<Value, Value> >;
